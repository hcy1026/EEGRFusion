...
from baselines.eegmamba import EEGMamba
from baselines.neurolm import NeuroLM
from baselines.ctnet import CTNet
from baselines.cbramod import CBraMod
from baselines.eegminer import EEGMiner
from baselines.msvtnet import MSVTNet
from baselines.pbt import PBT
from baselines.eeginceptionmi import EEGInceptionMI
from baselines.eegnex import EEGNex
import numpy as np
...
# ===================== NEW: significance + coverage@confidence =====================
def _logsumexp(log_vals):
    """Stable log(sum(exp(log_vals)))."""
    if len(log_vals) == 0:
        return -float("inf")
    m = max(log_vals)
...
def binomial_p_value_one_sided_greater(hits: int, n: int, p0: float) -> float:
    """
    One-sided p-value for H1: acc > p0 under H0: hits ~ Binomial(n, p0).
    Returns P(Hits >= hits_observed).
    这里等价于你说的“置换检验(随机猜测)”在 2-way/4-way forced-choice 场景下的精确版本。
    """
    if n <= 0:
        return 1.0
    hits = int(hits)
...
def coverage_at_confidence(margins, corrects, coverages=(0.1, 0.25, 0.5)):
    """
    margins: top1-top2 作为置信度
    corrects: 0/1
    coverages: 选取“最高置信度”的比例（例如0.1=top10%）
    返回 acc@covXX / covXX / tau@covXX
    """
    margins = np.asarray(margins, dtype=np.float64)
    corrects = np.asarray(corrects, dtype=np.int32)

    finite = np.isfinite(margins)
    margins = margins[finite]
    corrects = corrects[finite]

    n = int(margins.size)
    if n == 0:
        return {}

    order = np.argsort(-margins)  # descending
    out = {}
    for cov in coverages:
        ...
        out[f"acc@cov{q}"] = float(corrects[idx].mean())
        out[f"cov{q}"] = float(m / n)
        out[f"tau@cov{q}"] = float(margins[idx[-1]])  # cutoff margin
    return out
# ================================================================================


def evaluate_model(sub, eeg_model, dataloader, device, text_features_all, img_features_all, k, config,
                   extra_metrics: bool = False, conf_coverages=(0.5, 0.75)):
    eeg_model.eval()
    ...
    top5_acc = 0
    # NEW: 2-way/4-way 需要记录 margin 和每个 trial 的正确与否
    margins = []
    correct_flags = []
    with torch.no_grad():
        for batch_idx, (eeg_data, labels, text, text_features, img, img_features) in enumerate(dataloader):
            ...
            for idx, label in enumerate(labels):
                ...
                if k==200:
                    ...
                    # predicted_label = selected_classes[torch.argmax(logits_single).item()]
                    predicted_label = selected_classes[torch.argmax(logits_single).item()] # (n_batch, ) in {0, 1, ..., n_cls-1}
                    is_correct = int(predicted_label == label.item())
                    if predicted_label == label.item():
                        correct += 1
                    total += 1

                    # NEW: 只对 2-way/4-way 收集 margin 和正确性，用于 coverage@confidence
                    if extra_metrics and (k == 2 or k == 4):
                        top2 = torch.topk(logits_single, 2, largest=True).values
    ...
    # NEW: 返回额外指标（只在 k=2/4 且 extra_metrics=True 时）
    if extra_metrics and (k == 2 or k == 4):
        chance = 1.0 / float(k)
        p_value = binomial_p_value_one_sided_greater(hits=int(correct), n=int(total), p0=chance)

        se = math.sqrt(chance * (1.0 - chance) / total) if total > 0 else float("inf")
        z = (accuracy - chance) / se if (total > 0 and se > 0) else float("inf")
        ...
        extras.update(coverage_at_confidence(margins, correct_flags, coverages=conf_coverages))
        return average_loss, accuracy, top5_acc, extras
    ...
    for epoch in range(config.epochs):
        ...
        if (epoch +1) % 5 == 0:                    
            ...
            if config.insubject==True:       
                ...
                save_dir = os.path.join(config.output_dir, config.encoder_type, sub, current_time)
                ...
                save_dir = os.path.join(config.output_dir, config.encoder_type, "across", current_time)
                os.makedirs(save_dir, exist_ok=True)
                file_path = os.path.join(save_dir, f"{epoch + 1}.pth")
                torch.save(eeg_model.state_dict(), file_path)
        ...
        _, v2_acc, _, v2_extra = evaluate_model(sub, eeg_model, test_dataloader, device,
                                                text_features_test_all, img_features_test_all,
                                                k=2, config=config, extra_metrics=True)

        _, v4_acc, _, v4_extra = evaluate_model(sub, eeg_model, test_dataloader, device,
                                                text_features_test_all, img_features_test_all,
                                                k=4, config=config, extra_metrics=True)
        ...
        epoch_results = {
            ...
            "v2_p": v2_extra.get("p_value", None),
            "v2_z": v2_extra.get("z", None),
            "v2_n": v2_extra.get("n_trials", None),
            "v2_acc@cov50": v2_extra.get("acc@cov50", None),
            "v2_tau@cov50": v2_extra.get("tau@cov50", None),
            "v2_acc@cov75": v2_extra.get("acc@cov75", None),
            "v2_tau@cov75": v2_extra.get("tau@cov75", None),

            "v4_p": v4_extra.get("p_value", None),
            "v4_z": v4_extra.get("z", None),
            "v4_n": v4_extra.get("n_trials", None),
            "v4_acc@cov50": v4_extra.get("acc@cov50", None),
            "v4_tau@cov50": v4_extra.get("tau@cov50", None),
            "v4_acc@cov75": v4_extra.get("acc@cov75", None),
            "v4_tau@cov75": v4_extra.get("tau@cov75", None),
        }
        ...
        print(
            ...
            f"v2 acc={v2_acc:.4f} p={v2_extra.get('p_value', float('nan')):.3e} "
            f"acc@cov50={v2_extra.get('acc@cov50', float('nan')):.4f} | "
            f"v4 acc={v4_acc:.4f} p={v4_extra.get('p_value', float('nan')):.3e} "
            f"acc@cov50={v4_extra.get('acc@cov50', float('nan')):.4f} | "
            f"v10 acc={v10_acc:.4f} - v50 acc={v50_acc:.4f} - v100 acc={v100_acc:.4f}"
        )
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='../Retrieval/outputs/contrast', help='Directory to save output results')
    ...
    # parser.add_argument('--encoder_type', type=str, default='EEGMamba', help='Encoder type')  # 很低
    # parser.add_argument('--encoder_type', type=str, default='EEGMiner', help='Encoder type')  # 很低
    # parser.add_argument('--encoder_type', type=str, default='CBraMod', help='Encoder type')  # 第三个
    # parser.add_argument('--encoder_type', type=str, default='NeuroLM', help='Encoder type')  # 很低
    # parser.add_argument('--encoder_type', type=str, default='EEGInceptionMI', help='Encoder type')  # 第四个
    parser.add_argument('--encoder_type', type=str, default='EEGNex', help='Encoder type')  # 第六个
    parser.add_argument('--subjects', nargs='+', default=['sub-03', 'sub-04',
                                                          'sub-05', 'sub-06', 'sub-07', 'sub-08', 'sub-09', 'sub-10'], help='List of subject IDs (default: sub-01 to sub-10)')
    # parser.add_argument('--subjects', nargs='+', default=['sub-02'], help='List of subject IDs (default: sub-02)')
    ...
    for sub in subjects:
        ...
        total_params = count_params(eeg_model, trainable_only=False)
        trainable_params = count_params(eeg_model, trainable_only=True)

        encoder_params = count_params(eeg_model.encoder, trainable_only=True)
        backbone_params = count_params(eeg_model.encoder.encoder, trainable_only=True)

        # print to terminal
        print("\n========== Model Parameters ==========")
        print(f"Encoder type      : {args.encoder_type}")
        print(f"Subject           : {sub}")
        print(f"Total params      : {total_params} ({format_millions(total_params)})")
        print(f"Trainable params  : {trainable_params} ({format_millions(trainable_params)})")
        print(f"Encoder params    : {encoder_params} ({format_millions(encoder_params)})")
        print(f"Backbone(transformer): {backbone_params} ({format_millions(backbone_params)})")
        print("======================================\n")

        # save to output directory (same as csv)
        param_save_dir = os.path.join(args.output_dir, args.encoder_type, sub, current_time)
        os.makedirs(param_save_dir, exist_ok=True)

        param_file = os.path.join(
            param_save_dir,
            f"model_params_{args.encoder_type}_{sub}.txt"
        )
        ...
        with open(param_file, "w") as f:
            ...
            f.write(f"Total params     : {total_params} ({format_millions(total_params)})\n")
            f.write(f"Trainable params : {trainable_params} ({format_millions(trainable_params)})\n")
            f.write(f"Encoder params   : {encoder_params} ({format_millions(encoder_params)})\n")
            f.write(f"Backbone(transformer)  : {backbone_params} ({format_millions(backbone_params)})\n")

        print(f"[Param saved] {param_file}")
        # ==========================================================
