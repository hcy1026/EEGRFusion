...
def seed_everything(seed: int = 42, deterministic: bool = True) -> int:
    ...
    try:
        ...
        if deterministic:
            ...
            # Make some CUDA ops deterministic (may affect performance)
            os.environ.setdefault("CUBLAS_WORKSPACE_CONFIG", ":4096:8")

            # PyTorch deterministic algorithms (best effort)
            try:
        ...
        # torch not installed in this env
        pass
...
def seed_worker(worker_id: int):
    """
    Use this as DataLoader(worker_init_fn=seed_worker) for reproducible workers.
    """
    worker_seed = (torch.initial_seed() % 2**32) if "torch" in globals() else (worker_id % 2**32)
